import statemind from "../icons/statemind.svg";
import ethereum from "../icons/ethereum.svg";
import certora from "../icons/certora.svg";
import mixbytes from "../icons/mixbytes.svg";
import hexens from "../icons/hexens.svg";
import sigmaprime from "../icons/sigmaprime.svg";
import chainsecurity from "../icons/chainsecurity.svg";
import oxorio from "../icons/oxorio.svg";
import polygon from "../icons/polygon.svg";

export default function Autits() {
  return (
    <section
      data-track-content="true"
      data-content-name="Audits"
      data-content-piece="Audits carousel"
      className="lajEYW"
    >
      <div className="jPEekX cGzKkK">
        <h2 color="#ffffff" className="bOUZCY">
          Audits
        </h2>
        <p color="#ffffff" className="gXMcFO">
          Lido protocol has been audited by the industry-leaders in blockchain
          security.
        </p>
        <div className="bOLcvx">
          <div
            className="swiper swiper-initialized swiper-horizontal swiper-pointer-events"
            style={{ overflow: "visible" }}
          >
            <div
              className="swiper-wrapper"
              style={{
                transitionDuration: "0ms",
                transform: "translate3d(-4270.5px, 0px, 0px)",
              }}
            >
              <div
                className="swiper-slide swiper-slide-duplicate swiper-slide-duplicate-active"
                data-swiper-slide-index={0}
                style={{ width: "auto", marginRight: 32 }}
              >
                <div className="jHDxPB">
                  <div className="jQPYdt">
                    <img src={statemind} alt="" />
                    <img src={ethereum} alt="" className="glkIfC" />
                  </div>
                  <div className="iralfL">
                    <span className="cscJOh">Lido on Ethereum</span>
                    <span className="ixlxyR">&nbsp;|&nbsp;</span>
                    <span className="kyBwjH">May 2023</span>
                  </div>
                  <p className="eEjbzj">
                    StateMind Lido V2 Upgrade Template Audit
                  </p>
                  <a
                    target="_blank"
                    rel="noreferrer"
                    href="https://github.com/lidofinance/audits/blob/main/Statemind%20Lido%20V2%20Upgrade%20Template%20Audit%20Report%2005-2023.pdf"
                    className="czRlZm"
                  >
                    See report
                  </a>
                </div>
              </div>
              <div
                className="swiper-slide swiper-slide-duplicate swiper-slide-duplicate-next"
                data-swiper-slide-index={1}
                style={{ width: "auto", marginRight: 32 }}
              >
                <div className="jHDxPB">
                  <div className="jQPYdt">
                    <img src={statemind} alt="" />
                    <img src={ethereum} alt="" className="glkIfC" />
                  </div>
                  <div className="iralfL">
                    <span className="cscJOh">Lido on Ethereum</span>
                    <span className="ixlxyR">&nbsp;|&nbsp;</span>
                    <span className="kyBwjH">May 2023</span>
                  </div>
                  <p className="eEjbzj">
                    StateMind Lido V2 Deployment Validation Note
                  </p>
                  <a
                    target="_blank"
                    rel="noreferrer"
                    href="https://github.com/lidofinance/audits/blob/main/Statemind%20Lido%20V2%20Deployment%20Validation%2005-2023.pdf"
                    className="czRlZm"
                  >
                    See report
                  </a>
                </div>
              </div>
              <div
                className="swiper-slide swiper-slide-duplicate"
                data-swiper-slide-index={2}
                style={{ width: "auto", marginRight: 32 }}
              >
                <div className="jHDxPB">
                  <div className="jQPYdt">
                    <img src={statemind} alt="" />
                    <img src={ethereum} alt="" className="glkIfC" />
                  </div>
                  <div className="iralfL">
                    <span className="cscJOh">Lido on Ethereum</span>
                    <span className="ixlxyR">&nbsp;|&nbsp;</span>
                    <span className="kyBwjH">Apr 2023</span>
                  </div>
                  <p className="eEjbzj">StateMind Lido V2 Audit</p>
                  <a
                    target="_blank"
                    rel="noreferrer"
                    href="https://github.com/lidofinance/audits/blob/main/Statemind%20Lido%20V2%20Audit%20Report%2004-23.pdf"
                    className="czRlZm"
                  >
                    See report
                  </a>
                </div>
              </div>
              <div
                className="swiper-slide swiper-slide-duplicate"
                data-swiper-slide-index={3}
                style={{ width: "auto", marginRight: 32 }}
              >
                <div className="jHDxPB">
                  <div className="jQPYdt">
                    <img src={certora} alt="" />
                    <img src={ethereum} alt="" className="glkIfC" />
                  </div>
                  <div className="iralfL">
                    <span className="cscJOh">Lido on Ethereum</span>
                    <span className="ixlxyR">&nbsp;|&nbsp;</span>
                    <span className="kyBwjH">Apr 2023</span>
                  </div>
                  <p className="eEjbzj">Certora Lido V2 Audit</p>
                  <a
                    target="_blank"
                    rel="noreferrer"
                    href="https://github.com/lidofinance/audits/blob/main/Certora%20Lido%20V2%20Audit%20Report%2004-23.pdf"
                    className="czRlZm"
                  >
                    See report
                  </a>
                </div>
              </div>
              <div
                className="swiper-slide swiper-slide-duplicate"
                data-swiper-slide-index={4}
                style={{ width: "auto", marginRight: 32 }}
              >
                <div className="jHDxPB">
                  <div className="jQPYdt">
                    <img src={statemind} alt="" />
                    <img src={ethereum} alt="" className="glkIfC" />
                  </div>
                  <div className="iralfL">
                    <span className="cscJOh">Lido on Ethereum</span>
                    <span className="ixlxyR">&nbsp;|&nbsp;</span>
                    <span className="kyBwjH">Apr 2023</span>
                  </div>
                  <p className="eEjbzj">StateMind GateSeals Audit</p>
                  <a
                    target="_blank"
                    rel="noreferrer"
                    href="https://github.com/lidofinance/audits/blob/main/Statemind%20GateSeals%20Audit%20Report%2004-2023.pdf"
                    className="czRlZm"
                  >
                    See report
                  </a>
                </div>
              </div>
              <div
                className="swiper-slide swiper-slide-duplicate"
                data-swiper-slide-index={5}
                style={{ width: "auto", marginRight: 32 }}
              >
                <div className="jHDxPB">
                  <div className="jQPYdt">
                    <img src={mixbytes} alt="" />
                    <img src={ethereum} alt="" className="glkIfC" />
                  </div>
                  <div className="iralfL">
                    <span className="cscJOh">Lido on Ethereum</span>
                    <span className="ixlxyR">&nbsp;|&nbsp;</span>
                    <span className="kyBwjH">Apr 2023</span>
                  </div>
                  <p className="eEjbzj">MixBytes Camp Lido V2 Contest Report</p>
                  <a
                    target="_blank"
                    rel="noreferrer"
                    href="https://github.com/lidofinance/audits/blob/main/MixBytes%20Camp%20Lido%20V2%20Contest%20Report%2004-23.pdf"
                    className="czRlZm"
                  >
                    See report
                  </a>
                </div>
              </div>
              <div
                className="swiper-slide swiper-slide-duplicate"
                data-swiper-slide-index={6}
                style={{ width: "auto", marginRight: 32 }}
              >
                <div className="jHDxPB">
                  <div className="jQPYdt">
                    <img src={hexens} alt="" />
                    <img src={ethereum} alt="" className="glkIfC" />
                  </div>
                  <div className="iralfL">
                    <span className="cscJOh">Lido on Ethereum</span>
                    <span className="ixlxyR">&nbsp;|&nbsp;</span>
                    <span className="kyBwjH">Apr 2023</span>
                  </div>
                  <p className="eEjbzj">Hexens Lido V2 Smart Contract Audit</p>
                  <a
                    target="_blank"
                    rel="noreferrer"
                    href="https://github.com/lidofinance/audits/blob/main/Hexens%20Lido%20V2%20Smart%20Contract%20Audit%20Report%2004-23.pdf"
                    className="czRlZm"
                  >
                    See report
                  </a>
                </div>
              </div>
              <div
                className="swiper-slide swiper-slide-duplicate"
                data-swiper-slide-index={7}
                style={{ width: "auto", marginRight: 32 }}
              >
                <div className="jHDxPB">
                  <div className="jQPYdt">
                    <img src={sigmaprime} alt="" />
                    <img src={ethereum} alt="" className="glkIfC" />
                  </div>
                  <div className="iralfL">
                    <span className="cscJOh">Lido on Ethereum</span>
                    <span className="ixlxyR">&nbsp;|&nbsp;</span>
                    <span className="kyBwjH">Mar 2023</span>
                  </div>
                  <p className="eEjbzj">Sigma Prime dc4bc Security Audit</p>
                  <a
                    target="_blank"
                    rel="noreferrer"
                    href="https://github.com/lidofinance/audits/blob/main/Sigma%20Prime%20-%20Lido%20-%20dc4bc%20Security%20Assessment%20Report%20-%20v2.1%2003-2023.pdf"
                    className="czRlZm"
                  >
                    See report
                  </a>
                </div>
              </div>
              <div
                className="swiper-slide swiper-slide-duplicate"
                data-swiper-slide-index={8}
                style={{ width: "auto", marginRight: 32 }}
              >
                <div className="jHDxPB">
                  <div className="jQPYdt">
                    <img src={chainsecurity} alt="" />
                    <img src={ethereum} alt="" className="glkIfC" />
                  </div>
                  <div className="iralfL">
                    <span className="cscJOh">Lido on Ethereum</span>
                    <span className="ixlxyR">&nbsp;|&nbsp;</span>
                    <span className="kyBwjH">Feb 2023</span>
                  </div>
                  <p className="eEjbzj">
                    ChainSecurity Lido Staking Router audit report
                  </p>
                  <a
                    target="_blank"
                    rel="noreferrer"
                    href="https://github.com/lidofinance/audits/blob/main/ChainSecurity%20Lido%20Staking%20Router%20audit%20report%2002-23.pdf"
                    className="czRlZm"
                  >
                    See report
                  </a>
                </div>
              </div>
              <div
                className="swiper-slide swiper-slide-duplicate"
                data-swiper-slide-index={9}
                style={{ width: "auto", marginRight: 32 }}
              >
                <div className="jHDxPB">
                  <div className="jQPYdt">
                    <img src={statemind} alt="" />
                    <img src={ethereum} alt="" className="glkIfC" />
                  </div>
                  <div className="iralfL">
                    <span className="cscJOh">Lido on Ethereum</span>
                    <span className="ixlxyR">&nbsp;|&nbsp;</span>
                    <span className="kyBwjH">Jan 2023</span>
                  </div>
                  <p className="eEjbzj">
                    StateMind TRP Vesting Escrow Audit Report
                  </p>
                  <a
                    target="_blank"
                    rel="noreferrer"
                    href="https://github.com/lidofinance/audits/blob/main/Statemind%20TRP%20Vesting%20Escrow%20Audit%20Report%2001-2023.pdf"
                    className="czRlZm"
                  >
                    See report
                  </a>
                </div>
              </div>
              <div
                className="swiper-slide swiper-slide-duplicate"
                data-swiper-slide-index={10}
                style={{ width: "auto", marginRight: 32 }}
              >
                <div className="jHDxPB">
                  <div className="jQPYdt">
                    <img src={statemind} alt="" />
                    <img src={ethereum} alt="" className="glkIfC" />
                  </div>
                  <div className="iralfL">
                    <span className="cscJOh">Lido on Ethereum</span>
                    <span className="ixlxyR">&nbsp;|&nbsp;</span>
                    <span className="kyBwjH">Sep 2022</span>
                  </div>
                  <p className="eEjbzj">
                    MEV-Boost Relay Allowlist Security Audit Report
                  </p>
                  <a
                    target="_blank"
                    rel="noreferrer"
                    href="https://github.com/lidofinance/audits/blob/main/Statemind%20MEV-Boost%20relay%20allowlist%20Security%20Audit%20Report%2009-2022.pdf"
                    className="czRlZm"
                  >
                    See report
                  </a>
                </div>
              </div>
              <div
                className="swiper-slide swiper-slide-duplicate"
                data-swiper-slide-index={11}
                style={{ width: "auto", marginRight: 32 }}
              >
                <div className="jHDxPB">
                  <div className="jQPYdt">
                    <img src={chainsecurity} alt="" />
                    <img src={ethereum} alt="" className="glkIfC" />
                  </div>
                  <div className="iralfL">
                    <span className="cscJOh">Lido on Ethereum</span>
                    <span className="ixlxyR">&nbsp;|&nbsp;</span>
                    <span className="kyBwjH">Aug 2022</span>
                  </div>
                  <p className="eEjbzj">
                    Code Assessment Of The Lido Smart Contracts
                  </p>
                  <a
                    target="_blank"
                    rel="noreferrer"
                    href="https://github.com/lidofinance/audits/blob/main/ChainSecurity%20Code%20Assessment%20of%20the%20Lido%20Smart%20Contracts%20Report%2008-22.pdf"
                    className="czRlZm"
                  >
                    See report
                  </a>
                </div>
              </div>
              <div
                className="swiper-slide swiper-slide-duplicate"
                data-swiper-slide-index={12}
                style={{ width: "auto", marginRight: 32 }}
              >
                <div className="jHDxPB">
                  <div className="jQPYdt">
                    <img src={oxorio} alt="" />
                    <img src={polygon} alt="" className="glkIfC" />
                  </div>
                  <div className="iralfL">
                    <span className="cscJOh">Lido on Polygon</span>
                    <span className="ixlxyR">&nbsp;|&nbsp;</span>
                    <span className="kyBwjH">Aug 2022</span>
                  </div>
                  <p className="eEjbzj">Lido On Polygon V2 Upgrade Audit</p>
                  <a
                    target="_blank"
                    rel="noreferrer"
                    href="https://github.com/lidofinance/audits/blob/main/polygon/Oxorio%20Lido%20on%20Polygon%20V2%2008-2022.pdf"
                    className="czRlZm"
                  >
                    See report
                  </a>
                </div>
              </div>
              <div
                className="swiper-slide swiper-slide-duplicate swiper-slide-prev"
                data-swiper-slide-index={13}
                style={{ width: "auto", marginRight: 32 }}
              >
                <div className="jHDxPB">
                  <div className="jQPYdt">
                    <img src={oxorio} alt="" />
                    <img src={polygon} alt="" className="glkIfC" />
                  </div>
                  <div className="iralfL">
                    <span className="cscJOh">Lido on Polygon</span>
                    <span className="ixlxyR">&nbsp;|&nbsp;</span>
                    <span className="kyBwjH">Apr 2022</span>
                  </div>
                  <p className="eEjbzj">
                    Lido On Polygon Security Audit PR# 69
                  </p>
                  <a
                    target="_blank"
                    rel="noreferrer"
                    href="https://github.com/lidofinance/audits/blob/main/polygon/Oxorio%20Lido%20on%20Polygon%20pr69%20report%2004-2022.pdf"
                    className="czRlZm"
                  >
                    See report
                  </a>
                </div>
              </div>
              <div
                className="swiper-slide swiper-slide-active"
                data-swiper-slide-index={0}
                style={{ width: "auto", marginRight: 32 }}
              >
                <div className="jHDxPB">
                  <div className="jQPYdt">
                    <img src={statemind} alt="" />
                    <img src={ethereum} alt="" className="glkIfC" />
                  </div>
                  <div className="iralfL">
                    <span className="cscJOh">Lido on Ethereum</span>
                    <span className="ixlxyR">&nbsp;|&nbsp;</span>
                    <span className="kyBwjH">May 2023</span>
                  </div>
                  <p className="eEjbzj">
                    StateMind Lido V2 Upgrade Template Audit
                  </p>
                  <a
                    target="_blank"
                    rel="noreferrer"
                    href="https://github.com/lidofinance/audits/blob/main/Statemind%20Lido%20V2%20Upgrade%20Template%20Audit%20Report%2005-2023.pdf"
                    className="czRlZm"
                  >
                    See report
                  </a>
                </div>
              </div>
              <div
                className="swiper-slide swiper-slide-next"
                data-swiper-slide-index={1}
                style={{ width: "auto", marginRight: 32 }}
              >
                <div className="jHDxPB">
                  <div className="jQPYdt">
                    <img src={statemind} alt="" />
                    <img src={ethereum} alt="" className="glkIfC" />
                  </div>
                  <div className="iralfL">
                    <span className="cscJOh">Lido on Ethereum</span>
                    <span className="ixlxyR">&nbsp;|&nbsp;</span>
                    <span className="kyBwjH">May 2023</span>
                  </div>
                  <p className="eEjbzj">
                    StateMind Lido V2 Deployment Validation Note
                  </p>
                  <a
                    target="_blank"
                    rel="noreferrer"
                    href="https://github.com/lidofinance/audits/blob/main/Statemind%20Lido%20V2%20Deployment%20Validation%2005-2023.pdf"
                    className="czRlZm"
                  >
                    See report
                  </a>
                </div>
              </div>
              <div
                className="swiper-slide"
                data-swiper-slide-index={2}
                style={{ width: "auto", marginRight: 32 }}
              >
                <div className="jHDxPB">
                  <div className="jQPYdt">
                    <img src={statemind} alt="" />
                    <img src={ethereum} alt="" className="glkIfC" />
                  </div>
                  <div className="iralfL">
                    <span className="cscJOh">Lido on Ethereum</span>
                    <span className="ixlxyR">&nbsp;|&nbsp;</span>
                    <span className="kyBwjH">Apr 2023</span>
                  </div>
                  <p className="eEjbzj">StateMind Lido V2 Audit</p>
                  <a
                    target="_blank"
                    rel="noreferrer"
                    href="https://github.com/lidofinance/audits/blob/main/Statemind%20Lido%20V2%20Audit%20Report%2004-23.pdf"
                    className="czRlZm"
                  >
                    See report
                  </a>
                </div>
              </div>
              <div
                className="swiper-slide"
                data-swiper-slide-index={3}
                style={{ width: "auto", marginRight: 32 }}
              >
                <div className="jHDxPB">
                  <div className="jQPYdt">
                    <img src={certora} alt="" />
                    <img src={ethereum} alt="" className="glkIfC" />
                  </div>
                  <div className="iralfL">
                    <span className="cscJOh">Lido on Ethereum</span>
                    <span className="ixlxyR">&nbsp;|&nbsp;</span>
                    <span className="kyBwjH">Apr 2023</span>
                  </div>
                  <p className="eEjbzj">Certora Lido V2 Audit</p>
                  <a
                    target="_blank"
                    rel="noreferrer"
                    href="https://github.com/lidofinance/audits/blob/main/Certora%20Lido%20V2%20Audit%20Report%2004-23.pdf"
                    className="czRlZm"
                  >
                    See report
                  </a>
                </div>
              </div>
              <div
                className="swiper-slide"
                data-swiper-slide-index={4}
                style={{ width: "auto", marginRight: 32 }}
              >
                <div className="jHDxPB">
                  <div className="jQPYdt">
                    <img src={statemind} alt="" />
                    <img src={ethereum} alt="" className="glkIfC" />
                  </div>
                  <div className="iralfL">
                    <span className="cscJOh">Lido on Ethereum</span>
                    <span className="ixlxyR">&nbsp;|&nbsp;</span>
                    <span className="kyBwjH">Apr 2023</span>
                  </div>
                  <p className="eEjbzj">StateMind GateSeals Audit</p>
                  <a
                    target="_blank"
                    rel="noreferrer"
                    href="https://github.com/lidofinance/audits/blob/main/Statemind%20GateSeals%20Audit%20Report%2004-2023.pdf"
                    className="czRlZm"
                  >
                    See report
                  </a>
                </div>
              </div>
              <div
                className="swiper-slide"
                data-swiper-slide-index={5}
                style={{ width: "auto", marginRight: 32 }}
              >
                <div className="jHDxPB">
                  <div className="jQPYdt">
                    <img src={mixbytes} alt="" />
                    <img src={ethereum} alt="" className="glkIfC" />
                  </div>
                  <div className="iralfL">
                    <span className="cscJOh">Lido on Ethereum</span>
                    <span className="ixlxyR">&nbsp;|&nbsp;</span>
                    <span className="kyBwjH">Apr 2023</span>
                  </div>
                  <p className="eEjbzj">MixBytes Camp Lido V2 Contest Report</p>
                  <a
                    target="_blank"
                    rel="noreferrer"
                    href="https://github.com/lidofinance/audits/blob/main/MixBytes%20Camp%20Lido%20V2%20Contest%20Report%2004-23.pdf"
                    className="czRlZm"
                  >
                    See report
                  </a>
                </div>
              </div>
              <div
                className="swiper-slide"
                data-swiper-slide-index={6}
                style={{ width: "auto", marginRight: 32 }}
              >
                <div className="jHDxPB">
                  <div className="jQPYdt">
                    <img src={hexens} alt="" />
                    <img src={ethereum} alt="" className="glkIfC" />
                  </div>
                  <div className="iralfL">
                    <span className="cscJOh">Lido on Ethereum</span>
                    <span className="ixlxyR">&nbsp;|&nbsp;</span>
                    <span className="kyBwjH">Apr 2023</span>
                  </div>
                  <p className="eEjbzj">Hexens Lido V2 Smart Contract Audit</p>
                  <a
                    target="_blank"
                    rel="noreferrer"
                    href="https://github.com/lidofinance/audits/blob/main/Hexens%20Lido%20V2%20Smart%20Contract%20Audit%20Report%2004-23.pdf"
                    className="czRlZm"
                  >
                    See report
                  </a>
                </div>
              </div>
              <div
                className="swiper-slide"
                data-swiper-slide-index={7}
                style={{ width: "auto", marginRight: 32 }}
              >
                <div className="jHDxPB">
                  <div className="jQPYdt">
                    <img src={sigmaprime} alt="" />
                    <img src={ethereum} alt="" className="glkIfC" />
                  </div>
                  <div className="iralfL">
                    <span className="cscJOh">Lido on Ethereum</span>
                    <span className="ixlxyR">&nbsp;|&nbsp;</span>
                    <span className="kyBwjH">Mar 2023</span>
                  </div>
                  <p className="eEjbzj">Sigma Prime dc4bc Security Audit</p>
                  <a
                    target="_blank"
                    rel="noreferrer"
                    href="https://github.com/lidofinance/audits/blob/main/Sigma%20Prime%20-%20Lido%20-%20dc4bc%20Security%20Assessment%20Report%20-%20v2.1%2003-2023.pdf"
                    className="czRlZm"
                  >
                    See report
                  </a>
                </div>
              </div>
              <div
                className="swiper-slide"
                data-swiper-slide-index={8}
                style={{ width: "auto", marginRight: 32 }}
              >
                <div className="jHDxPB">
                  <div className="jQPYdt">
                    <img src={chainsecurity} alt="" />
                    <img src={ethereum} alt="" className="glkIfC" />
                  </div>
                  <div className="iralfL">
                    <span className="cscJOh">Lido on Ethereum</span>
                    <span className="ixlxyR">&nbsp;|&nbsp;</span>
                    <span className="kyBwjH">Feb 2023</span>
                  </div>
                  <p className="eEjbzj">
                    ChainSecurity Lido Staking Router audit report
                  </p>
                  <a
                    target="_blank"
                    rel="noreferrer"
                    href="https://github.com/lidofinance/audits/blob/main/ChainSecurity%20Lido%20Staking%20Router%20audit%20report%2002-23.pdf"
                    className="czRlZm"
                  >
                    See report
                  </a>
                </div>
              </div>
              <div
                className="swiper-slide"
                data-swiper-slide-index={9}
                style={{ width: "auto", marginRight: 32 }}
              >
                <div className="jHDxPB">
                  <div className="jQPYdt">
                    <img src={statemind} alt="" />
                    <img src={ethereum} alt="" className="glkIfC" />
                  </div>
                  <div className="iralfL">
                    <span className="cscJOh">Lido on Ethereum</span>
                    <span className="ixlxyR">&nbsp;|&nbsp;</span>
                    <span className="kyBwjH">Jan 2023</span>
                  </div>
                  <p className="eEjbzj">
                    StateMind TRP Vesting Escrow Audit Report
                  </p>
                  <a
                    target="_blank"
                    rel="noreferrer"
                    href="https://github.com/lidofinance/audits/blob/main/Statemind%20TRP%20Vesting%20Escrow%20Audit%20Report%2001-2023.pdf"
                    className="czRlZm"
                  >
                    See report
                  </a>
                </div>
              </div>
              <div
                className="swiper-slide"
                data-swiper-slide-index={10}
                style={{ width: "auto", marginRight: 32 }}
              >
                <div className="jHDxPB">
                  <div className="jQPYdt">
                    <img src={statemind} alt="" />
                    <img src={ethereum} alt="" className="glkIfC" />
                  </div>
                  <div className="iralfL">
                    <span className="cscJOh">Lido on Ethereum</span>
                    <span className="ixlxyR">&nbsp;|&nbsp;</span>
                    <span className="kyBwjH">Sep 2022</span>
                  </div>
                  <p className="eEjbzj">
                    MEV-Boost Relay Allowlist Security Audit Report
                  </p>
                  <a
                    target="_blank"
                    rel="noreferrer"
                    href="https://github.com/lidofinance/audits/blob/main/Statemind%20MEV-Boost%20relay%20allowlist%20Security%20Audit%20Report%2009-2022.pdf"
                    className="czRlZm"
                  >
                    See report
                  </a>
                </div>
              </div>
              <div
                className="swiper-slide"
                data-swiper-slide-index={11}
                style={{ width: "auto", marginRight: 32 }}
              >
                <div className="jHDxPB">
                  <div className="jQPYdt">
                    <img src={chainsecurity} alt="" />
                    <img src={ethereum} alt="" className="glkIfC" />
                  </div>
                  <div className="iralfL">
                    <span className="cscJOh">Lido on Ethereum</span>
                    <span className="ixlxyR">&nbsp;|&nbsp;</span>
                    <span className="kyBwjH">Aug 2022</span>
                  </div>
                  <p className="eEjbzj">
                    Code Assessment Of The Lido Smart Contracts
                  </p>
                  <a
                    target="_blank"
                    rel="noreferrer"
                    href="https://github.com/lidofinance/audits/blob/main/ChainSecurity%20Code%20Assessment%20of%20the%20Lido%20Smart%20Contracts%20Report%2008-22.pdf"
                    className="czRlZm"
                  >
                    See report
                  </a>
                </div>
              </div>
              <div
                className="swiper-slide"
                data-swiper-slide-index={12}
                style={{ width: "auto", marginRight: 32 }}
              >
                <div className="jHDxPB">
                  <div className="jQPYdt">
                    <img src={oxorio} alt="" />
                    <img src={polygon} alt="" className="glkIfC" />
                  </div>
                  <div className="iralfL">
                    <span className="cscJOh">Lido on Polygon</span>
                    <span className="ixlxyR">&nbsp;|&nbsp;</span>
                    <span className="kyBwjH">Aug 2022</span>
                  </div>
                  <p className="eEjbzj">Lido On Polygon V2 Upgrade Audit</p>
                  <a
                    target="_blank"
                    rel="noreferrer"
                    href="https://github.com/lidofinance/audits/blob/main/polygon/Oxorio%20Lido%20on%20Polygon%20V2%2008-2022.pdf"
                    className="czRlZm"
                  >
                    See report
                  </a>
                </div>
              </div>
              <div
                className="swiper-slide swiper-slide-duplicate-prev"
                data-swiper-slide-index={13}
                style={{ width: "auto", marginRight: 32 }}
              >
                <div className="jHDxPB">
                  <div className="jQPYdt">
                    <img src={oxorio} alt="" />
                    <img src={polygon} alt="" className="glkIfC" />
                  </div>
                  <div className="iralfL">
                    <span className="cscJOh">Lido on Polygon</span>
                    <span className="ixlxyR">&nbsp;|&nbsp;</span>
                    <span className="kyBwjH">Apr 2022</span>
                  </div>
                  <p className="eEjbzj">
                    Lido On Polygon Security Audit PR# 69
                  </p>
                  <a
                    target="_blank"
                    rel="noreferrer"
                    href="https://github.com/lidofinance/audits/blob/main/polygon/Oxorio%20Lido%20on%20Polygon%20pr69%20report%2004-2022.pdf"
                    className="czRlZm"
                  >
                    See report
                  </a>
                </div>
              </div>
              <div
                className="swiper-slide swiper-slide-duplicate swiper-slide-duplicate-active"
                data-swiper-slide-index={0}
                style={{ width: "auto", marginRight: 32 }}
              >
                <div className="jHDxPB">
                  <div className="jQPYdt">
                    <img src={statemind} alt="" />
                    <img src={ethereum} alt="" className="glkIfC" />
                  </div>
                  <div className="iralfL">
                    <span className="cscJOh">Lido on Ethereum</span>
                    <span className="ixlxyR">&nbsp;|&nbsp;</span>
                    <span className="kyBwjH">May 2023</span>
                  </div>
                  <p className="eEjbzj">
                    StateMind Lido V2 Upgrade Template Audit
                  </p>
                  <a
                    target="_blank"
                    rel="noreferrer"
                    href="https://github.com/lidofinance/audits/blob/main/Statemind%20Lido%20V2%20Upgrade%20Template%20Audit%20Report%2005-2023.pdf"
                    className="czRlZm"
                  >
                    See report
                  </a>
                </div>
              </div>
              <div
                className="swiper-slide swiper-slide-duplicate swiper-slide-duplicate-next"
                data-swiper-slide-index={1}
                style={{ width: "auto", marginRight: 32 }}
              >
                <div className="jHDxPB">
                  <div className="jQPYdt">
                    <img src={statemind} alt="" />
                    <img src={ethereum} alt="" className="glkIfC" />
                  </div>
                  <div className="iralfL">
                    <span className="cscJOh">Lido on Ethereum</span>
                    <span className="ixlxyR">&nbsp;|&nbsp;</span>
                    <span className="kyBwjH">May 2023</span>
                  </div>
                  <p className="eEjbzj">
                    StateMind Lido V2 Deployment Validation Note
                  </p>
                  <a
                    target="_blank"
                    rel="noreferrer"
                    href="https://github.com/lidofinance/audits/blob/main/Statemind%20Lido%20V2%20Deployment%20Validation%2005-2023.pdf"
                    className="czRlZm"
                  >
                    See report
                  </a>
                </div>
              </div>
              <div
                className="swiper-slide swiper-slide-duplicate"
                data-swiper-slide-index={2}
                style={{ width: "auto", marginRight: 32 }}
              >
                <div className="jHDxPB">
                  <div className="jQPYdt">
                    <img src={statemind} alt="" />
                    <img src={ethereum} alt="" className="glkIfC" />
                  </div>
                  <div className="iralfL">
                    <span className="cscJOh">Lido on Ethereum</span>
                    <span className="ixlxyR">&nbsp;|&nbsp;</span>
                    <span className="kyBwjH">Apr 2023</span>
                  </div>
                  <p className="eEjbzj">StateMind Lido V2 Audit</p>
                  <a
                    target="_blank"
                    rel="noreferrer"
                    href="https://github.com/lidofinance/audits/blob/main/Statemind%20Lido%20V2%20Audit%20Report%2004-23.pdf"
                    className="czRlZm"
                  >
                    See report
                  </a>
                </div>
              </div>
              <div
                className="swiper-slide swiper-slide-duplicate"
                data-swiper-slide-index={3}
                style={{ width: "auto", marginRight: 32 }}
              >
                <div className="jHDxPB">
                  <div className="jQPYdt">
                    <img src={certora} alt="" />
                    <img src={ethereum} alt="" className="glkIfC" />
                  </div>
                  <div className="iralfL">
                    <span className="cscJOh">Lido on Ethereum</span>
                    <span className="ixlxyR">&nbsp;|&nbsp;</span>
                    <span className="kyBwjH">Apr 2023</span>
                  </div>
                  <p className="eEjbzj">Certora Lido V2 Audit</p>
                  <a
                    target="_blank"
                    rel="noreferrer"
                    href="https://github.com/lidofinance/audits/blob/main/Certora%20Lido%20V2%20Audit%20Report%2004-23.pdf"
                    className="czRlZm"
                  >
                    See report
                  </a>
                </div>
              </div>
              <div
                className="swiper-slide swiper-slide-duplicate"
                data-swiper-slide-index={4}
                style={{ width: "auto", marginRight: 32 }}
              >
                <div className="jHDxPB">
                  <div className="jQPYdt">
                    <img src={statemind} alt="" />
                    <img src={ethereum} alt="" className="glkIfC" />
                  </div>
                  <div className="iralfL">
                    <span className="cscJOh">Lido on Ethereum</span>
                    <span className="ixlxyR">&nbsp;|&nbsp;</span>
                    <span className="kyBwjH">Apr 2023</span>
                  </div>
                  <p className="eEjbzj">StateMind GateSeals Audit</p>
                  <a
                    target="_blank"
                    rel="noreferrer"
                    href="https://github.com/lidofinance/audits/blob/main/Statemind%20GateSeals%20Audit%20Report%2004-2023.pdf"
                    className="czRlZm"
                  >
                    See report
                  </a>
                </div>
              </div>
              <div
                className="swiper-slide swiper-slide-duplicate"
                data-swiper-slide-index={5}
                style={{ width: "auto", marginRight: 32 }}
              >
                <div className="jHDxPB">
                  <div className="jQPYdt">
                    <img src={mixbytes} alt="" />
                    <img src={ethereum} alt="" className="glkIfC" />
                  </div>
                  <div className="iralfL">
                    <span className="cscJOh">Lido on Ethereum</span>
                    <span className="ixlxyR">&nbsp;|&nbsp;</span>
                    <span className="kyBwjH">Apr 2023</span>
                  </div>
                  <p className="eEjbzj">MixBytes Camp Lido V2 Contest Report</p>
                  <a
                    target="_blank"
                    rel="noreferrer"
                    href="https://github.com/lidofinance/audits/blob/main/MixBytes%20Camp%20Lido%20V2%20Contest%20Report%2004-23.pdf"
                    className="czRlZm"
                  >
                    See report
                  </a>
                </div>
              </div>
              <div
                className="swiper-slide swiper-slide-duplicate"
                data-swiper-slide-index={6}
                style={{ width: "auto", marginRight: 32 }}
              >
                <div className="jHDxPB">
                  <div className="jQPYdt">
                    <img src={hexens} alt="" />
                    <img src={ethereum} alt="" className="glkIfC" />
                  </div>
                  <div className="iralfL">
                    <span className="cscJOh">Lido on Ethereum</span>
                    <span className="ixlxyR">&nbsp;|&nbsp;</span>
                    <span className="kyBwjH">Apr 2023</span>
                  </div>
                  <p className="eEjbzj">Hexens Lido V2 Smart Contract Audit</p>
                  <a
                    target="_blank"
                    rel="noreferrer"
                    href="https://github.com/lidofinance/audits/blob/main/Hexens%20Lido%20V2%20Smart%20Contract%20Audit%20Report%2004-23.pdf"
                    className="czRlZm"
                  >
                    See report
                  </a>
                </div>
              </div>
              <div
                className="swiper-slide swiper-slide-duplicate"
                data-swiper-slide-index={7}
                style={{ width: "auto", marginRight: 32 }}
              >
                <div className="jHDxPB">
                  <div className="jQPYdt">
                    <img src={sigmaprime} alt="" />
                    <img src={ethereum} alt="" className="glkIfC" />
                  </div>
                  <div className="iralfL">
                    <span className="cscJOh">Lido on Ethereum</span>
                    <span className="ixlxyR">&nbsp;|&nbsp;</span>
                    <span className="kyBwjH">Mar 2023</span>
                  </div>
                  <p className="eEjbzj">Sigma Prime dc4bc Security Audit</p>
                  <a
                    target="_blank"
                    rel="noreferrer"
                    href="https://github.com/lidofinance/audits/blob/main/Sigma%20Prime%20-%20Lido%20-%20dc4bc%20Security%20Assessment%20Report%20-%20v2.1%2003-2023.pdf"
                    className="czRlZm"
                  >
                    See report
                  </a>
                </div>
              </div>
              <div
                className="swiper-slide swiper-slide-duplicate"
                data-swiper-slide-index={8}
                style={{ width: "auto", marginRight: 32 }}
              >
                <div className="jHDxPB">
                  <div className="jQPYdt">
                    <img src={chainsecurity} alt="" />
                    <img src={ethereum} alt="" className="glkIfC" />
                  </div>
                  <div className="iralfL">
                    <span className="cscJOh">Lido on Ethereum</span>
                    <span className="ixlxyR">&nbsp;|&nbsp;</span>
                    <span className="kyBwjH">Feb 2023</span>
                  </div>
                  <p className="eEjbzj">
                    ChainSecurity Lido Staking Router audit report
                  </p>
                  <a
                    target="_blank"
                    rel="noreferrer"
                    href="https://github.com/lidofinance/audits/blob/main/ChainSecurity%20Lido%20Staking%20Router%20audit%20report%2002-23.pdf"
                    className="czRlZm"
                  >
                    See report
                  </a>
                </div>
              </div>
              <div
                className="swiper-slide swiper-slide-duplicate"
                data-swiper-slide-index={9}
                style={{ width: "auto", marginRight: 32 }}
              >
                <div className="jHDxPB">
                  <div className="jQPYdt">
                    <img src={statemind} alt="" />
                    <img src={ethereum} alt="" className="glkIfC" />
                  </div>
                  <div className="iralfL">
                    <span className="cscJOh">Lido on Ethereum</span>
                    <span className="ixlxyR">&nbsp;|&nbsp;</span>
                    <span className="kyBwjH">Jan 2023</span>
                  </div>
                  <p className="eEjbzj">
                    StateMind TRP Vesting Escrow Audit Report
                  </p>
                  <a
                    target="_blank"
                    rel="noreferrer"
                    href="https://github.com/lidofinance/audits/blob/main/Statemind%20TRP%20Vesting%20Escrow%20Audit%20Report%2001-2023.pdf"
                    className="czRlZm"
                  >
                    See report
                  </a>
                </div>
              </div>
              <div
                className="swiper-slide swiper-slide-duplicate"
                data-swiper-slide-index={10}
                style={{ width: "auto", marginRight: 32 }}
              >
                <div className="jHDxPB">
                  <div className="jQPYdt">
                    <img src={statemind} alt="" />
                    <img src={ethereum} alt="" className="glkIfC" />
                  </div>
                  <div className="iralfL">
                    <span className="cscJOh">Lido on Ethereum</span>
                    <span className="ixlxyR">&nbsp;|&nbsp;</span>
                    <span className="kyBwjH">Sep 2022</span>
                  </div>
                  <p className="eEjbzj">
                    MEV-Boost Relay Allowlist Security Audit Report
                  </p>
                  <a
                    target="_blank"
                    rel="noreferrer"
                    href="https://github.com/lidofinance/audits/blob/main/Statemind%20MEV-Boost%20relay%20allowlist%20Security%20Audit%20Report%2009-2022.pdf"
                    className="czRlZm"
                  >
                    See report
                  </a>
                </div>
              </div>
              <div
                className="swiper-slide swiper-slide-duplicate"
                data-swiper-slide-index={11}
                style={{ width: "auto", marginRight: 32 }}
              >
                <div className="jHDxPB">
                  <div className="jQPYdt">
                    <img src={chainsecurity} alt="" />
                    <img src={ethereum} alt="" className="glkIfC" />
                  </div>
                  <div className="iralfL">
                    <span className="cscJOh">Lido on Ethereum</span>
                    <span className="ixlxyR">&nbsp;|&nbsp;</span>
                    <span className="kyBwjH">Aug 2022</span>
                  </div>
                  <p className="eEjbzj">
                    Code Assessment Of The Lido Smart Contracts
                  </p>
                  <a
                    target="_blank"
                    rel="noreferrer"
                    href="https://github.com/lidofinance/audits/blob/main/ChainSecurity%20Code%20Assessment%20of%20the%20Lido%20Smart%20Contracts%20Report%2008-22.pdf"
                    className="czRlZm"
                  >
                    See report
                  </a>
                </div>
              </div>
              <div
                className="swiper-slide swiper-slide-duplicate"
                data-swiper-slide-index={12}
                style={{ width: "auto", marginRight: 32 }}
              >
                <div className="jHDxPB">
                  <div className="jQPYdt">
                    <img src={oxorio} alt="" />
                    <img src={polygon} alt="" className="glkIfC" />
                  </div>
                  <div className="iralfL">
                    <span className="cscJOh">Lido on Polygon</span>
                    <span className="ixlxyR">&nbsp;|&nbsp;</span>
                    <span className="kyBwjH">Aug 2022</span>
                  </div>
                  <p className="eEjbzj">Lido On Polygon V2 Upgrade Audit</p>
                  <a
                    target="_blank"
                    rel="noreferrer"
                    href="https://github.com/lidofinance/audits/blob/main/polygon/Oxorio%20Lido%20on%20Polygon%20V2%2008-2022.pdf"
                    className="czRlZm"
                  >
                    See report
                  </a>
                </div>
              </div>
              <div
                className="swiper-slide swiper-slide-duplicate"
                data-swiper-slide-index={13}
                style={{ width: "auto", marginRight: 32 }}
              >
                <div className="jHDxPB">
                  <div className="jQPYdt">
                    <img src={oxorio} alt="" />
                    <img src={polygon} alt="" className="glkIfC" />
                  </div>
                  <div className="iralfL">
                    <span className="cscJOh">Lido on Polygon</span>
                    <span className="ixlxyR">&nbsp;|&nbsp;</span>
                    <span className="kyBwjH">Apr 2022</span>
                  </div>
                  <p className="eEjbzj">
                    Lido On Polygon Security Audit PR# 69
                  </p>
                  <a
                    target="_blank"
                    rel="noreferrer"
                    href="https://github.com/lidofinance/audits/blob/main/polygon/Oxorio%20Lido%20on%20Polygon%20pr69%20report%2004-2022.pdf"
                    className="czRlZm"
                  >
                    See report
                  </a>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="nQjWw">
          <button aria-label="Previous audit slide" className="fKOoFv humxiE">
            <svg
              width={6}
              height={10}
              viewBox="0 0 6 10"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                fillRule="evenodd"
                clipRule="evenodd"
                d="M0.292893 5.70711C-0.097631 5.31658 -0.097631 4.68342 0.292893 4.29289L4.29289 0.292894C4.68342 -0.0976312 5.31658 -0.0976312 5.70711 0.292894C6.09763 0.683418 6.09763 1.31658 5.70711 1.70711L2.41421 5L5.70711 8.29289C6.09763 8.68342 6.09763 9.31658 5.70711 9.70711C5.31658 10.0976 4.68342 10.0976 4.29289 9.70711L0.292893 5.70711Z"
                fill="white"
              />
            </svg>
          </button>
          <a
            href="https://github.com/lidofinance/audits"
            target="_blank"
            rel="noreferrer"
            className="lmlpeK oHOZr"
          >
            <span className="ebQdAR">Explore all audits</span>
            <span className="jOkYuc">Explore all</span>
          </a>
          <button aria-label="Next audit slide" className="fKOoFv humxiE">
            <svg
              width={6}
              height={10}
              viewBox="0 0 6 10"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                fillRule="evenodd"
                clipRule="evenodd"
                d="M5.70711 5.70711C6.09763 5.31658 6.09763 4.68342 5.70711 4.29289L1.70711 0.292894C1.31658 -0.0976312 0.683417 -0.0976312 0.292893 0.292894C-0.0976311 0.683418 -0.0976311 1.31658 0.292893 1.70711L3.58579 5L0.292894 8.29289C-0.0976307 8.68342 -0.0976307 9.31658 0.292894 9.70711C0.683418 10.0976 1.31658 10.0976 1.70711 9.70711L5.70711 5.70711Z"
                fill="white"
              />
            </svg>
          </button>
        </div>
      </div>
    </section>
  );
}
